...
module.exports = function handler(req, res) {
  try {
    ...
    // In Vercel, the file will be in the dist/spa/.well-known directory after build
    ...
    try {
      ...
      const distPath = path.join(process.cwd(), 'dist', 'spa', '.well-known', 'apple-developer-merchantid-domain-association');
      try {
        fileContent = fs.readFileSync(distPath, 'utf8');
      } catch (distError) {
        console.error('File not found in public or dist:', readError, distError);
