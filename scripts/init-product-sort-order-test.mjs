...
/**
 * Initialize sortOrder for all existing products in TEST environment
 */
...
console.log(`Initializing sortOrder for products in TEST project: ${firebaseConfig.projectId}`);
...
async function initializeProductSortOrder() {
  try {
    ...
    for (const key of Object.keys(groupedProducts)) {
      const [category, collectionName] = key.split('::');
      console.log(`  Category: ${category}, Collection: ${collectionName}`);
